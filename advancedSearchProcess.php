    ...
    if(isset($_POST["s"])) {
        ...
        if(empty($s)) {
            ...
            if(!empty($s) && $c!=0) {
                ...
                $rs_pro_pg = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `category_id`='".$c."';");

            } else if(!empty($s) && $b!=0) {
                $ss = "2-".$sort."-".$b;

                $rs_pro_pg = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `model_has_brand_id` 
                ...
                $rs_pro_pg = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `condition_id`='".$con."';");

            } else if(!empty($s) && $col!=0) {
                $ss = "5-".$sort."-".$col;
                
                $rs_pro_pg = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `color_id`='".$col."';");

            } else if(!empty($s) && !empty($pf) && empty($pt)) {
                $ss = "6-".$sort."-".$pf;

                $rs_pro_pg = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `price`>='".$pf."';");

            } else if(!empty($s) && !empty($pt) && empty($pf)) {
                $ss = "7-".$sort."-".$pt;

                $rs_pro_pg = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `price`<='".$pt."';");

            } else if(!empty($s) && !empty($pf) && !empty($pt)) {
                $ss = "8-".$sort."-".$pf."-".$pt;

                $rs_pro_pg = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `price`>='".$pf."' AND `price`<='".$pt."';");

            } else if(!empty($s)) {
                $ss = "0-".$sort;

                $rs_pro_pg = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%';");
            }
            // Find product count for pagination
            ...
            if(!empty($s) && $c!=0) {
                $rs_pro = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `category_id`='".$c."'
                ORDER BY ".$order." LIMIT 6 OFFSET ".$offset.";");
            } else if(!empty($s) && $b!=0) {
                $rs_pro = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `model_has_brand_id` 
                ...
                $rs_pro = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `condition_id`='".$con."'
                ORDER BY ".$order." LIMIT 6 OFFSET ".$offset.";");
            } else if(!empty($s) && $col!=0) {
                $rs_pro = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `color_id`='".$col."'
                ORDER BY ".$order." LIMIT 6 OFFSET ".$offset.";");
            } else if(!empty($s) && !empty($pf) && empty($pt)) {
                $rs_pro = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `price`>='".$pf."'
                ORDER BY ".$order." LIMIT 6 OFFSET ".$offset.";");
            } else if(!empty($s) && !empty($pt) && empty($pf)) {
                $rs_pro = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `price`<='".$pt."'
                ORDER BY ".$order." LIMIT 6 OFFSET ".$offset.";");
            } else if(!empty($s) && !empty($pf) && !empty($pt)) {
                $rs_pro = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' AND `price`>='".$pf."' AND `price`<='".$pt."'
                ORDER BY ".$order." LIMIT 6 OFFSET ".$offset.";");
            } else if(!empty($s)) {
                $rs_pro = Database::search("SELECT * FROM `product` WHERE `description`LIKE'%".$s."%' ORDER BY ".$order." LIMIT 6 
                OFFSET ".$offset.";");
            }
            ...
            if($nr_pro>=1) {
                for($i=0; $i<$nr_pro; $i++) {
                    ...
                    $rs_img = Database::search("SELECT * FROM `images` WHERE `product_id`='".$pro['id']."' LIMIT 1;");
                    $nr_img = $rs_img->num_rows;
                    if($nr_img==1) {
                        $img = $rs_img->fetch_assoc();
                        $pro["img"] =  $img["code"];
                    } else {
                        $pro["img"] = 'resources/no_image.jpg';
                    }
                    $array[$i] = $pro;
                }
                                    ...
                                    <div class="card mb-3">
                                        <div class="row g-0">
                                            <div class="col-md-4 mt-4">
                                                <img src="<?php echo $array[$j]["img"]; ?>" class="img-fluid rounded-start" style="height: 100px;"/>
                                            </div>
                                            <div class="col-md-8">
                                                <div class="card-body">

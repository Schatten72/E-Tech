...
if (isset($_SESSION["u"])) {
    ...
    $cityS = Database::search("SELECT * FROM `user_has_address` WHERE `user_email`='" . $userEmail . "' ;");
    ...
    if ($citySNr == 1) {
        ...
        for ($i = 0; $i < $size; $i++) {
            ...
            Database::iud("UPDATE `cart` SET `qty`='".$qty."' WHERE `product_id`='".$id."' AND  `user_id`='".$userEmail."';");

            $data = Database::search("SELECT * FROM `product` WHERE `id`='" . $id . "' ");
            ...
            if ($data->num_rows == 1) {
                ...
                if ($citySNr == 1) {
                    ...
                    $districtS = Database::search("SELECT * FROM `city` WHERE `id`='" . $cityId . "' ;");
                    $districtSD = $districtS->fetch_assoc();
                    $districtId = $districtSD["district_id"];

                    $districtNameS = Database::search("SELECT * FROM `district` WHERE `id`='" . $districtId . "' ;");
                    $districtNameSD = $districtNameS->fetch_assoc();

                    if ($districtId == 1) {
                        $dilivery = $productSD["delivery_fee_other"];
                    } else {
                        $dilivery = $productSD["delivery_fee_colombo"];
                    }
                }
